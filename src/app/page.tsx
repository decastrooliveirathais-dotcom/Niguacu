...
import { Users, GraduationCap, FileCheck, FileText, TrendingUp, Search, RefreshCw } from 'lucide-react'
...
export default function Dashboard() {
  ...
  const [filtros, setFiltros] = useState({ curso: 'todos', turno: 'todos', fase: 'todos', flagFin: 'todos', flagDoc: 'todos', flagAcad: 'todos', busca: '' })
  ...
  const clear = () => { setFiltros({ curso: 'todos', turno: 'todos', fase: 'todos', flagFin: 'todos', flagDoc: 'todos', flagAcad: 'todos', busca: '' }); setPagina(1) }
  ...
  return (
    <div className="min-h-screen bg-gray-100 p-3 md:p-4">
      <div className="max-w-7xl mx-auto">
        ...
        {/* KPIs */}
        {loading && !data ? (
          <div className="grid grid-cols-2 md:grid-cols-5 gap-2 mb-4">{[...Array(5)].map((_, i) => <Card key={i} className="shadow-sm"><CardContent className="py-4"><Skeleton className="h-12 w-full" /></CardContent></Card>)}</div>
        ) : (
          <div className="grid grid-cols-2 md:grid-cols-5 gap-2 mb-4">
            <KPI titulo="Total" valor={data?.kpis?.total || 0} desc="Oportunidades" icon={Users} color="bg-blue-500" />
            <KPI titulo="Mat. Financeira" valor={data?.kpis?.matFin || 0} desc="Pagamentos" icon={FileCheck} color="bg-green-500" />
            <KPI titulo="Mat. Acadêmica" valor={data?.kpis?.matAcad || 0} desc="Finalizadas" icon={GraduationCap} color="bg-emerald-500" />
            <KPI titulo="Documentação" valor={data?.kpis?.docEntregue || 0} desc="Entregues" icon={FileText} color="bg-amber-500" />
            <KPI titulo="Conversão" valor={`${data?.kpis?.conversao || 0}%`} desc="Taxa" icon={TrendingUp} color="bg-purple-500" />
          </div>
        )}
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-3 mb-4">
          ...
          <Card className="shadow-sm bg-gradient-to-br from-blue-50 to-green-50">
            ...
            <CardContent className="pb-3 px-3 text-sm space-y-1.5">
              <div className="flex justify-between"><span className="text-gray-500">Filtrado:</span><b>{(data?.kpis?.total || 0).toLocaleString('pt-BR')}</b></div>
              <div className="flex justify-between"><span className="text-gray-500">Conversão:</span><Badge className="bg-green-500 text-[10px]">{data?.kpis?.conversao}%</Badge></div>
          ...
          <CardContent className="px-3 pb-3">
            <div className="overflow-x-auto">
              <table className="w-full text-xs">
                ...
                <tbody>
                  {loading ? [...Array(5)].map((_, i) => <tr key={i} className="border-b">{[...Array(8)].map((_, j) => <td key={j} className="p-1.5"><Skeleton className="h-3 w-full" /></td>)}</tr>) 
                  : data?.tabela?.dados?.map((r: any, i: number) => (
                    <tr key={i} className="border-b hover:bg-gray-50">
                      <td className="p-1.5 truncate max-w-[150px]">{r.curso}</td>
                      <td className="p-1.5">{r.turno}</td>
                      <td className="p-1.5"><Badge variant="outline" className="text-[10px]">{r.fase?.slice(0, 10)}</Badge></td>
